...
#[serde_as]
#[derive(Clone, Debug, Serialize, Deserialize)]
pub struct GenTable<T: Debug + Clone + PartialEq + Eq + Default + Into<Vec<u8>>, H: Hasher<T>> {
...
fn make_internal_func<T: HashResult, H: Hasher<T>>(opcode: Opcode, ty: FunctionType) -> GenFunction<T,H> {
    let wavm = vec![
        Instruction::simple(Opcode::InitFrame),
        Instruction::simple(opcode),
        Instruction::simple(Opcode::Return),
    ];
    GenFunction::new_from_wavm(wavm, ty, Vec::new())
}
...
pub struct GenModule<T: HashResult, H: Hasher<T>> {
    pub globals: Vec<Value>,
    pub memory: GenMemory<T,H>,
    pub tables: Vec<GenTable<T,H>>,
    #[serde(skip)]
    pub tables_merkle: GenMerkle<T,H>,
    pub funcs: Arc<Vec<GenFunction<T,H>>>,
    #[serde(skip)]
    pub funcs_merkle: Arc<GenMerkle<T,H>>,
...
impl<T: HashResult, H: Hasher<T>> GenModule<T,H> {
    fn from_binary(
        ...
        let mut code : Vec<GenFunction<T,H>> = Vec::new();
        let mut func_type_idxs: Vec<u32> = Vec::new();
        let mut memory : GenMemory<T,H> = GenMemory::default();
        let mut exports = HashMap::default();
        let mut tables : Vec<GenTable<T,H>> = Vec::new();
        ...
        for import in &bin.imports {
            if let TypeRef::Func(ty) = import.ty {
                ...
                if let Some(import) = available_imports.get(&qualified_name) {
                    ...
                    func = GenFunction::new_from_wavm(wavm, import.ty.clone(), Vec::new());
                } else {
        ...
        for table in &bin.tables {
            tables.push(GenTable {
                ...
                ty: *table,
                elems_merkle: GenMerkle::default(),
            });
        }
        ...
        let tables_hashes: Result<_, _> = tables.iter().map(GenTable::hash).collect();

        Ok(Self {
            memory,
            globals,
            tables_merkle: GenMerkle::new(MerkleType::Table, tables_hashes?),
            tables,
...
impl GlobalState {
    ...
    pub fn gen_hash<T, H: Hasher<T>>(&self) -> T {
        let mut h = H::make();
        h.update_title(b"Global state:");
        for item in self.bytes32_vals {
            h.update_bytes32(&item)
        }
...
pub struct GenMachineState<'a, T: HashResult, H: Hasher<T>> {
    ...
    modules: Vec<GenModuleState<'a,T,H>>,
...
#[derive(Clone)]
pub struct GenMachine<T: HashResult, H: Hasher<T>> {
    steps: u64, // Not part of machine hash
    status: MachineStatus,
    pub value_stack: Vec<Value>,
    pub internal_stack: Vec<Value>,
    pub block_stack: Vec<usize>,
    pub frame_stack: Vec<StackFrame>,
    pub modules: Vec<GenModule<T,H>>,
    modules_merkle: Option<GenMerkle<T,H>>,
...
pub fn gen_hash_stack<T: HashResult, H: Hasher<T>, I>(stack: I, prefix: &[u8]) -> T
where
...
fn hash_value_stack(stack: &[Value]) -> Bytes32 {
    gen_hash_value_stack::<Bytes32,Keccak>(stack)
}

pub fn gen_hash_value_stack<T: HashResult, H: Hasher<T>>(stack: &[Value]) -> T {
    gen_hash_stack::<T,H,_>(stack.iter().map(|v| v.gen_hash::<T,H>()), b"Value stack:")
}

fn hash_pc_stack(pcs: &[usize]) -> Bytes32 {
    gen_hash_pc_stack::<Bytes32,Keccak>(pcs)
}

pub fn gen_hash_pc_stack<T: HashResult, H: Hasher<T>>(pcs: &[usize]) -> T {
    gen_hash_stack::<T,H,_>(
        pcs.iter().map(|pc| T::from(*pc as u32)),
        b"Program counter stack:",
    )
}

fn hash_stack_frame_stack(frames: &[StackFrame]) -> Bytes32 {
    gen_hash_stack_frame_stack::<Bytes32,Keccak>(frames)
}

pub fn gen_hash_stack_frame_stack<T: HashResult, H: Hasher<T>>(frames: &[StackFrame]) -> T {
...
impl <T: HashResult + serde::de::DeserializeOwned, H: Hasher<T> + serde::de::DeserializeOwned + Serialize> GenMachine<T,H> {
    ...
    pub fn from_binaries(
        ...
        let mut modules = vec![GenModule::<T,H>::default()];
        ...
        for lib in libraries {
            let module : GenModule<T,H> = GenModule::from_binary(lib, &available_imports, &floating_point_impls, true)?;
        ...
        if always_merkleize {
            modules_merkle = Some(GenMerkle::new(
                MerkleType::Module,
                modules.iter().map(GenModule::hash).collect(),
            ));
        }
