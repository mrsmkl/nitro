...
impl<T: Debug + Clone + PartialEq + Eq + Default + Into<Vec<u8>>, H: Hasher<T>> GenMemory<T, H> {
    ...
    pub fn hash_leaf(bytes: [u8; Memory::LEAF_SIZE]) -> T {
        gen_hash_leaf::<T, H>(bytes)
    }

    pub fn merkelize(&self) -> Cow<'_, GenMerkle<T,H>> {
        if let Some(m) = &self.merkle {
            return Cow::Borrowed(m);
        }
        // Round the size up to 8 byte long leaves, then round up to the next power of two number of leaves
        let leaves = round_up_to_power_of_two(div_round_up(self.buffer.len(), Memory::LEAF_SIZE));
        let mut leaf_hashes: Vec<T> = self
            .buffer
            .chunks(Memory::LEAF_SIZE)
            .map(|leaf| {
                let mut full_leaf = [0u8; 32];
                full_leaf[..leaf.len()].copy_from_slice(leaf);
                Self::hash_leaf(full_leaf)
            })
            .collect();
        if leaf_hashes.len() < leaves {
            let empty_hash = Self::hash_leaf([0u8; 32]);
            leaf_hashes.resize(leaves, empty_hash);
        }
        Cow::Owned(GenMerkle::new_advanced(
            MerkleType::Memory,
            leaf_hashes,
            Self::hash_leaf([0u8; 32]),
            Memory::MEMORY_LAYERS,
        ))
    }

    pub fn get_leaf_data(&self, leaf_idx: usize) -> [u8; Memory::LEAF_SIZE] {
        let mut buf = [0u8; Memory::LEAF_SIZE];
        let idx = match leaf_idx.checked_mul(Memory::LEAF_SIZE) {
            Some(x) if x < self.buffer.len() => x,
            _ => return buf,
        };
        let size = std::cmp::min(Memory::LEAF_SIZE, self.buffer.len() - idx);
        buf[..size].copy_from_slice(&self.buffer[idx..(idx + size)]);
        buf
    }
    ...
    pub fn hash(&self) -> T {
        let mut h = H::make();
        h.update_title(b"Memory:");
        h.update_usize(self.buffer.len());
        h.update_hash(&self.merkelize().root());
        h.result()
    }
    ...
    pub fn store_value(&mut self, idx: u64, value: u64, bytes: u8) -> bool {
        ...
        if let Some(mut merkle) = self.merkle.take() {
            let start_leaf = idx / Memory::LEAF_SIZE;
            merkle.set(start_leaf, Self::hash_leaf(self.get_leaf_data(start_leaf)));
            let end_leaf = (end_idx - 1) / Memory::LEAF_SIZE;
            if end_leaf != start_leaf {
                merkle.set(end_leaf, Self::hash_leaf(self.get_leaf_data(end_leaf)));
            }
            self.merkle = Some(merkle);
        }
    ...
    #[must_use]
    pub fn store_slice_aligned(&mut self, idx: u64, value: &[u8]) -> bool {
        if idx % Memory::LEAF_SIZE as u64 != 0 {
            return false;
        }
        let end_idx = match idx.checked_add(value.len() as u64) {
            Some(x) => x,
        ...
        if let Some(mut merkle) = self.merkle.take() {
            ...
            merkle.set(start_leaf, Self::hash_leaf(self.get_leaf_data(start_leaf)));
            // No need for second merkle
            assert!(value.len() <= Memory::LEAF_SIZE);
        }
